...
/**
 * Calcula multa e juros para faturas vencidas.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Fetch penalty configuration from policies
    ...
    // Fetch overdue invoices (pending with due_date < today)
    const { data: overdueInvoices, error: fetchErr } = await supabase
      .from("invoices")
      .select("id, amount_cents, due_date, fine_amount_cents, interest_amount_cents, status")
      .in("status", ["pending", "overdue"])
      .lt("due_date", today);

    if (fetchErr) return json({ error: "Erro ao buscar faturas", details: fetchErr.message }, 500);
    if (!overdueInvoices || overdueInvoices.length === 0) {
      return json({ success: true, message: "Nenhuma fatura vencida", updated: 0 });
    }
    ...
    for (const inv of overdueInvoices) {
      ...
      // Calculate fine (one-time, 2% of amount)
      const fineAmountCents = Math.round(inv.amount_cents * (fineRatePct / 100));

      // Calculate interest (daily, capped at max monthly)
      ...
      // Change status to overdue if still pending
    ...
    console.log(`Penalties: ${updated} invoices updated, ${statusChanged} moved to overdue`);
