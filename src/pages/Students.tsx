...
import {
  ...
  type Student, type StudentFormData, type StudentStatusFilter,
...
export default function Students() {
  ...
  const handleCreate = () => { setEditingStudent(null); setDialogOpen(true); };
  const handleEdit = (student: Student) => { setEditingStudent(student); setDialogOpen(true); };

  const handleSubmit = (data: StudentFormData) => {
    if (editingStudent) {
      updateMutation.mutate({ id: editingStudent.id, data }, { onSuccess: () => setDialogOpen(false) });
    } else {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as StudentStatusFilter)}>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-muted-foreground">
          ...
          <p className="text-xs text-muted-foreground/70 mt-1">
            {search || statusFilter !== "all" ? "Tente ajustar os filtros" : "Comece adicionando seu primeiro aluno"}
          </p>
        </div>
      ) : (
                ...
                return (
                  <TableRow key={student.id} className="group cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/students/${student.id}`)}>
                    ...
                    <TableCell className="hidden lg:table-cell text-muted-foreground text-sm">
                      {student.date_of_birth ? format(new Date(student.date_of_birth + "T00:00:00"), "dd/MM/yyyy") : "—"}
                    </TableCell>
                    <TableCell>
      ...
      <StudentFormDialog open={dialogOpen} onOpenChange={setDialogOpen} student={editingStudent} onSubmit={handleSubmit} isSubmitting={isSubmitting} />
    </div>
  );
}
