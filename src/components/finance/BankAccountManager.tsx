            ...
            <Card key={acc.id} className={!acc.is_active ? "opacity-60" : ""}>
              <CardHeader className="pb-2 flex flex-row items-start justify-between">
                ...
                <div className="flex gap-1">
                  <Button size="icon" variant="ghost" aria-label={`Editar conta ${acc.name}`} className="h-7 w-7" onClick={() => { setEditing(acc); setDialogOpen(true); }}>
                    <Pencil className="h-3.5 w-3.5" />
                  </Button>
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button size="icon" variant="ghost" aria-label={`Excluir conta ${acc.name}`} className="h-7 w-7 text-destructive">
